...
try:
    ...
    PY_LANG = None
    print(f"[!] Python language binding not available: {e}")
...
try:
    ...
    JS_LANG = None
    print(f"[!] JavaScript language binding not available: {e}")
...
try:
    ...
    TS_LANG = None
    print(f"[!] TypeScript language binding not available: {e}")
...
try:
    ...
    GO_LANG = None
    print(f"[!] Go language binding not available: {e}")
...
LANGUAGE_CONFIGS = {
    ...
    '.js': {
        'name': 'javascript',
        'dangerous_functions': {'eval', 'exec', 'Function', 'child_process.exec', 'require', 'Buffer.from'},
    ...
    '.go': {
        'name': 'go',
        'dangerous_functions': {'exec.Command', 'os.Exec', 'syscall.Exec', 'net.Dial', 'sh'},
...
class TaintAnalyzer:
    """
    A language-agnostic taint analyzer using Tree-Sitter individual bindings.
    Tracks whether tainted data reaches dangerous sinks.
    """
        ...
        if HAS_TREE_SITTER:
            ...
            if lang is not None:
                try:
                    ...
                    print(f"[*] Tree-Sitter parser ready for {self.ext}")
                except Exception as e:
                    print(f"[!] Could not set language {self.language_name}: {e}")
                    self.parser = None
            else:
                print(f"[!] No Tree-Sitter binding found for {self.ext}")
                self.parser = None
    ...
    def analyze(self) -> Dict:
        ...
        if self.parser is not None:
            try:
                ...
                # Fall back to Python AST if this is Python
                if self.ext == '.py':
    ...
    def _traverse_node(self, node, source_code):
        """Recursively traverse Tree-Sitter AST."""
        try:
    ...
    def _analyze_python_ast(self):
        """Fallback: Use Python's built-in AST parser."""
        try:
            import ast
